...
WRFback=/scratch/admoore/WRFrun3/april_exps/cntl

# WRF2ARPS directory
wrf2arps=/scratch/admoore/wrf2arps2

# ARPS2WRF directory
arps2wrf=/scratch/admoore/arps2wrf2

# ARPS4WRF directory
arps4wrf=${arps}/arps4wrf/wrf2

# WRFOUT directory
wrfout=/scratch/admoore/wrfout2

# GFS met_em files directory
GFSmetem=/scratch/admoore
...
for hh in ${hours[@]}; do
	...
	then
        ...
        export startmin=${mm}
        export startday=20
        ${arps}/run/input/wrf2arps_input_uav2.sh
        #sbatch ${arps}/wrf2arps.sh
        ${arps}/bin/wrf2arps < ${arps}/run/input/wrf2arps2.input > wrf2arps2.out
	...
	# Choose the appropriate background error file. 
        # Background error improves as the cycling goes on
        if [ ${hh} -lt 14 ] 
        then
            export ErrNum=1
        elif [ ${hh} -lt 16 ] 
        ...
        #cd ${arps}
        ${arps}/run/input/adas_wrf_uav2.sh
	sbatch ${arps}/adas_para2.sh
	...
	#bin/arps3dvar < run/input/adas_wrf.input > 3dvar.out
        ...
        do
            echo "Waiting..." 
            sleep 3m  
            echo "Waited for $(( $((${counter}+1)) * 3 )) minutes"
	...
	sleep 30s
        ...
        ${arps}/run/input/arps4wrf_wrf_uav2.sh
        cd ${arps}
        #sbatch ${arps}/a4w.sh
        bin/arps4wrf < run/input/arps4wrf_wrf2.input > arps4wrf2.out
        cd ${WRFdir}        
        ...
        then
            ...
            ################### REAL ####################
                ...
                ${arps}/run/input/wrf2arps_input2.sh
                #sbatch ${arps}/wrf2arps.sh
	        ...
	        echo ""
                ...
                ${arps}/run/input/arps4wrf_wrf_lbc2.sh
                #sbatch ${arps}/a4w.sh
	...
	# NOTE: Currently the following section is set up to perform hourly DA
        # You will need to make appropriate adjustments to change it back to half hourly!        
         ...
         ################### WRF2ARPS ####################
            ...
            # Now we can pass this through ARPS4WRF
            ${arps}/run/input/arps4wrf_wrf_lbc_uav2.sh
            #sbatch ${arps}/a4w.sh
            echo "Submitted ARPS4WRF"
...
echo "Real success. Submitting WRF free forecast."

# Submit WRF forecast and exit
sbatch ${WRFdir}/wrfpp.sh
