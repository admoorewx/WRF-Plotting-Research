...
for hh in ${hours[@]}; do
	...
	then
        ...
        then
            ...
            echo "Beginning WRF pre-processing."
            export FirstCycle="True"
	    ...
	    # Going to run this through WRF for 1 min forecast.
	    # Process through WRF2ARPS
	    rm ${wrf2arps}/wrfassim_${hh}${mm}.hdfgrdbas 2> /dev/null
            rm ${wrf2arps}/wrfassim_${hh}${mm}* 2> /dev/null
            ...
            # Copy this output over to the directory where output from ADAS normally goes.
            cp ${wrf2arps}/wrfassim_${hh}${mm}.hdf000000 ${arps2wrf}/WRFASSIM_${hh}${mm}.hdf000000 2> /dev/null
            cp ${wrf2arps}/wrfassim_${hh}${mm}.hdfgrdbas ${arps2wrf}/WRFASSIM_${hh}${mm}.hdfgrdbas 2> /dev/null


            # Now to convert back to WRF format
            ...
            # Copy over the wrfinput file as a wrfout file
            ...
            rm ${wrf2arps}/wrfassim_${starthour}${startmin}.hdfgrdbas 2> /dev/null
            rm ${wrf2arps}/wrfassim_${starthour}${startmin}.hdf000000 2> /dev/null
            ...
            # Now copy the met_em files over to the WRF directory and run a 1-min forecast
            rm ${WRFdir}/met_em*
            cp ${arps4wrf}/met_em* ${WRFdir}
	    ...
	    # Export Values
            ...
            # create the one-minute namelist file
            ${WRFdir}/namelist_OneMin.sh
            # run real.exe
            ${WRFdir}/real.exe
            # Run WRF
            sbatch ${WRFdir}/wrfp.sh
            echo "  Submitted 1-min WRF."
            
            # Check for WRF output
            counter=0
            while [ ! -e ${WRFdir}/wrfout_d01_2013-05-20_${hh}:01:03 ]
            do
                echo "   waiting..."
                sleep 1m
                counter=$((${counter}+1))
	    ...
	    # Copy the output file over to the wrfout directory as the beginning DA time
            cp ${WRFdir}/wrfout_d01_2013-05-20_${hh}:01:03 ${wrfout}/wrfout_d01_2013-05-20_${hh}:00:00
            # Remove this file
            rm ${WRFdir}/wrfout*
            # Clean files from this process
            rm ${arps4wrf}/met*
            rm ${WRFdir}/met*
            rm ${wrf2arps}/wrfassim*
            rm ${arps2wrf}/WRFASSIM*
            rm ${WRFdir}/rsl*
            rm ${WRFdir}/bin_2d*
            # copy the GFS met_em files back to the WRF directory
            cp ${GFSmetem}/met* ${WRFdir}

	    echo "Done with the WRF-preprocessing forecast!"
            echo ""
        ...
        rm ${wrf2arps}/wrfassim_${hh}${mm}.hdfgrdbas 2> /dev/null
        rm ${wrf2arps}/wrfassim_${hh}${mm}.hdf000000 2> /dev/null
        ...
        counter=0 # Resetting the counter.
        while [ ! -e ${wrf2arps}/wrfassim_${hh}${mm}.hdf00* ]
        do
            echo "Waiting..."
            counter=$((${counter}+1))
            sleep 2m
        ...
        if [ ${FirstCycle} == "True" ] 
        then 
            echo "First Cycle is True."
            cp ${wrf2arps}/wrfassim_${hh}${mm}.hdf000063 ${wrf2arps}/wrfassim_${hh}${mm}.hdf000000
            export starttime=63
        else
            export starttime=00       
        ...
        rm ${arps2wrf}/WRFASSIM_${hh}${mm}.hdf000000 2> /dev/null
        rm ${arps2wrf}/WRFASSIM_${hh}${mm}.hdfgrdbas 2> /dev/null
	...
	#bin/arps3dvar < run/input/adas_wrf.input > 3dvar.out
        ...
        counter=0 # Resetting the counter
        while [ ! -e ${arps2wrf}/WRFASSIM_${hh}${mm}.hdf0000* ]
        do
            echo "Waiting..." 
            sleep 3m  
        ...
        if [ ${FirstCycle} == "True" ] 
        then 
            cp ${arps2wrf}/WRFASSIM_${hh}${mm}.hdf000063 ${arps2wrf}/WRFASSIM_${hh}${mm}.hdf000000
        fi
        ...
        rm ${arps4wrf}/met_em.d01.2013-05-20_${hh}:${mm}:00.nc 2> /dev/null
        rm ${arps4wrf}/met_em.d02.2013-05-20_${hh}:${mm}:00.nc 2> /dev/null
	...
	if [ ${FirstCycle} == "True" ] 
        ...
        # Need to make sure that ARPS4WRF ran correctly
        counter=0 # Resetting the counter
        while [ ! -e ${arps4wrf}/met_em.d01.2013-05-20_${hh}:${min}:${sec}.nc ]
        do
            echo "Waiting..."
            sleep 2m
            counter=$((${counter}+1))
...
################################### WRF Forecast #########################################################
	    ...
	    if [ ${FirstCycle} == "True" ] 
            ...
            cp ${WRFdir}/wrfout_d01_2013-05-20_${starthour}:${startmin}:${startsec} ${wrfout}/wrfout_d01_2013-05-20_${starthour}:00:00_AAA
